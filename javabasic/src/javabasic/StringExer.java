package javabasic; //javabasic 패키지 안에 있다

public class StringExer { //StringExer 클래스 안에 있다
	...
	public static void main(String[] args) { //main 메소드
		...
		// literal(리터럴)방식
		String str1 = "안녕하세요"; // String 타입에 str1변수에 안녕하세요를 저장
		
		// constructor(생성자) 방식
		char[] charArray = {'안', '녕', '하', '세', '요'}; // char 타입에 문자 한 개씩 넣어서 문자열을 만든다(배열 이용)
		String str2 = new String(charArray);
		
		System.out.println(str1); // str1 에 있는 문자열을 출력
		System.out.println(str2); // str2 에 있는 배열로 모은 문자들을 출력
		...
		System.out.println(str3.length());	// 10  //length = 길이
		
		// 반복문을 사용해서 문자열의 문자 추출 (코드 리팩토링 책)
		...
		//대입(Assignment) : 초기화한 후 변수에 값을 저장
		i = 200;
		...
		//문자열들이 메모리에 생성됨
		//StringBuffer 또는 StringBuilder를 사용해서 문자열 조작 작업을
		...
		System.out.println(str10.charAt(5)); 		//백   	// 0(동)부터 시작
		System.out.println(str10.contains("백두산")); //true  //포함하고 있나
		System.out.println(str10.endsWith("하느님이"));//false //끝나나
		
		System.out.println(str10.getBytes()); 		 //주소		
		...
		// 실습 4) exerStr 문장열을 공백 기준으로 분리하여 String[]로 변환해 출력
		...
		// 실습 5) exerStr 문장열을 공백 기준으로 분리하여 문자수가 3이상인 것들만 출력
		//1. 공백 기준으로 분리
